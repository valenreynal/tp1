...
{
    Repuesto.push_back(unRepuesto); ///agrego otro repuesto al final del vector
}

float Reparacion::getHorasTrabajadas()  ///muestra horas trabajadas
{
...
{
     float total= costoRepuestos* mecanic->getPrecio() * horasTrabajadas; ///al costo de los repuestos le sumo el precio x hora del mecanico y lo multiplico x las horas trabajadas
     total+=total*0.30; ///comision del 30%
     return total;
}
Reparacion::getMecanico()* const  ///muestro el mecanico asociado a esa reparaciom
{
    return mecanic;
}
